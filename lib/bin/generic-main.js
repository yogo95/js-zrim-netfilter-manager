...
Promise.resolve()
  ...
  .then(() => {
    return new Promise((resolve, reject) => {
      ...
      if (context.usageArgs.action === 'install') {
        _.each(context.processWorkflow.context.commands.ipset.add, cmd => {
          shFileData += `ipset ${cmd}\n`;
        });

        _.each(context.processWorkflow.context.commands.iptables.add, cmd => {
          shFileData += `iptables ${cmd}\n`;
        });
      } else {
        _.each(context.processWorkflow.context.commands.iptables.del, cmd => {
          shFileData += `iptables ${cmd}\n`;
        });

        _.each(context.processWorkflow.context.commands.ipset.del, cmd => {
          shFileData += `ipset ${cmd}\n`;
        });
      }
